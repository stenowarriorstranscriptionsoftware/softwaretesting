...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const bulkTestFile = document.getElementById('bulkTestFile');
  const uploadTestsBtn = document.getElementById('uploadTestsBtn');
  ...
  auth.onAuthStateChanged(user => {
    if (user) {
      ...
      // Check admin status and load admin tools if applicable
  ...
  function checkAdminStatus(userId) {
    return database.ref('admins').child(userId).once('value')
  ...
  createClassBtn.addEventListener('click', () => {
    ...
    const students = classStudents.value.trim()
      .split(',')
      .map(email => email.trim())
      .filter(email => email);
    
    if (!name || students.length === 0) {
      alert('Please enter a class name and at least one student email');
      return;
    }
  ...
  // Bulk test upload handler
  uploadTestsBtn.addEventListener('click', () => {
    const file = bulkTestFile.files[0];
    if (!file) {
    ...
    reader.onload = function(e) {
      ...
      const startRow = lines[0].includes('Title,Text') ? 1 : 0;
      
      for (let i = startRow; i < lines.length; i++) {
        if (lines[i].trim() === '') continue;
        
        const [title, ...textParts] = lines[i].split(',');
      ...
      if (tests.length === 0) {
        alert('No valid tests found in the CSV file');
        return;
      }
      ...
      tests.forEach(test => {
        const newTestKey = database.ref('tests').push().key;
        updates[`/tests/${newTestKey}`] = {
          title: test.title,
          text: test.text,
      ...
      database.ref().update(updates)
        .then(() => {
          alert(`Successfully uploaded ${tests.length} tests!`);
          bulkTestFile.value = '';
          loadGlobalTests();
        })
        .catch(error => {
          console.error('Error uploading tests:', error);
          alert('Failed to upload tests. Please try again.');
        });
    };
  ...
  // Class leaderboard filter handler
  classFilter.addEventListener('change', () => {
    ...
    database.ref(`classes/${classId}`).once('value')
      .then(snapshot => {
        ...
        database.ref('attempts').orderByChild('timestamp').once('value')
          .then(snapshot => {
            const attempts = snapshot.val();
            if (!attempts) {
              classLeaderboard.innerHTML = '<p>No attempts found for this class.</p>';
              return;
            }
            
            // Filter attempts by student emails
            const classAttempts = Object.entries(attempts)
              .map(([id, attempt]) => ({ id, ...attempt }))
              .filter(attempt => studentEmails.includes(attempt.userEmail));
            ...
            classAttempts.sort((a, b) => b.stats.accuracy - a.stats.accuracy);
            renderClassLeaderboard(classAttempts);
          });
      });
  });

  function renderClassLeaderboard(attempts) {
    let tableHTML = `
      <table>
        <thead>
          <tr>
    ...
    classLeaderboard.innerHTML = tableHTML;
  }
